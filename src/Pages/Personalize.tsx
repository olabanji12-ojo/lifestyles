...
// Assuming your Firebase configuration is exported from here
import { db } from '../firebase/config';
import toast from 'react-hot-toast';

// ✅ CORRECTION: Added 'Gift' icon and keeping only used icons clean
import { 
  Layers, User, DollarSign, ChevronDown, ChevronUp, Clock, CloudUpload, Gift 
} from 'lucide-react'; 

// --- CLOUDINARY CONFIGURATION (Replace these placeholders) ---
const CLOUD_NAME = 'YOUR_CLOUD_NAME'; // ← replace with your Cloudinary Cloud Name
const UPLOAD_PRESET = 'YOUR_UPLOAD_PRESET'; // ← replace with your unsigned preset name
...
// ✅ CORRECTION: Added '' to the state type to fix the TypeScript error (Code 2345)
type OpenSection = 'design' | 'details' | 'contact' | '';
...
  
  // State for Accordion control - Now uses the corrected OpenSection type
...
        throw new Error(error.message || 'Image upload failed. Check Cloudinary configuration.');
    }
  };
...
    e.preventDefault();
    
    // Simple required field check (Firebase addDoc will fail without this)
...
    
    try {
        // Handles image upload and returns URLs or throws an error
...
        toast.dismiss(loadingToastId); // Clear loading
        toast.success('Your request has been sent! We will contact you soon.');
        setDone(true);

    } catch (err: any) {
        toast.dismiss(loadingToastId); // Clear loading
        // The specific error message is thrown from uploadImages or firebase failure
        toast.error(err.message || 'Submission failed. Please check your network and fields.');
